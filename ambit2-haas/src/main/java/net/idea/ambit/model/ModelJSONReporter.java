...
public class ModelJSONReporter extends ModelURIReporterHaas {
	...
	enum jsonModel {
		URI, title, algorithm, trainingDataset, independent, dependent, predicted, legend, creator, mimetype, content, evaluations, algFormat, parameters, stars;
	...
	public void processItem(ModelQueryResults model, Writer output) {
		try {
			...
			getOutput().write(String.format("\n{" + "\n\"%s\":\"%s\"," + // uri
					...
					jsonModel.URI.jsonname(), uri, "id", model.getId(), jsonModel.title.jsonname(),
					JSONUtils.jsonEscape(model.getName()), jsonModel.stars.jsonname(), model.getStars(),
					jsonModel.algorithm.jsonname(),
					model.getAlgorithm() == null ? "" : JSONUtils.jsonEscape(model.getAlgorithm()),
					...
					jsonModel.trainingDataset.jsonname(),
					model.getTrainingInstances() == null ? "" : JSONUtils.jsonEscape(model.getTrainingInstances()),
					jsonModel.independent.jsonname(), String.format("%s/independent", uri),
					jsonModel.dependent.jsonname(), String.format("%s/dependent", uri), jsonModel.predicted.jsonname(),
					String.format("%s/predicted", uri),

					"ambitprop", jsonModel.legend.jsonname(), String.format("%s?media=image/png", uri),
					jsonModel.creator.jsonname(), JSONUtils.jsonEscape(model.getCreator()),
					jsonModel.mimetype.jsonname(), model.getContentMediaType(), jsonModel.content.jsonname(),
					"application/java".equals(model.getContentMediaType()) ? JSONUtils.jsonEscape(model.getContent())
							: (uri + "?media=text/plain"),
					jsonModel.evaluations.jsonname(), evals.toString(), jsonModel.stars.jsonname(), model.getStars(),
	...
	@Override
	public void footer(Writer output, Iterator<ModelQueryResults> query) {
		try {
			output.write("\n]\n}");
		} catch (Exception x) {
		}
	...
	@Override
	public void header(Writer output, Iterator<ModelQueryResults> query) {
		try {
			if (jsonpCallback != null) {
